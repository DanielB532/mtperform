          ...
          <nav className="hidden xl:flex items-center gap-6">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-xs transition-colors uppercase tracking-wider font-semibold text-secondary-foreground/80 hover:text-primary"
              >
                {link.label}
              </a>
            ))}
            ...
            <nav className="flex flex-col items-center gap-6 py-12">
              {navLinks.map((link) => (
                <a
                  ...
                  className="font-display text-xl text-secondary-foreground hover:text-primary transition-colors"
                >
                  {link.label}
                </a>
              ))}
